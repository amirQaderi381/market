                    ...
                    <table class="table table-striped table-hover h-150px">
                        ...
                        @foreach($product->images as $image)
                            <tr>
                                ...
                                <td>{{ $product->name }}</td>
                                <td><img src="{{ asset( $image->image['indexArray'][$image->image['currentImage']]) }}" width="100" height="50" alt=""></td>

                                <td class="max-width-16-rem text-left">
                                    <form action="{{ route('admin.market.gallery.destroy',['gallery'=>$image->id , 'product'=>$product->id]) }}" method="post" class="d-inline">
                                        @csrf
                                        @method('delete')
